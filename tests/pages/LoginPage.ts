...
export class LoginPage {
  ...
  async login(username: string, password: string) {
    ...
    await expect(
      this.page.getByTestId("current-user-name"),
      `Login should complete for ${username} before the test continues.`,
    ).toHaveText(username);
  }
  ...
  async expectCurrentUser(displayName: string) {
    await expect(
      this.page.getByTestId("current-user-name"),
      `현재 로그인 사용자가 ${displayName}이어야 합니다.`,
